...
ADD COLUMN "osmosis_symbol_total_volumes_in_usd" numeric NULL;
...
, prices as (
    SELECT DISTINCT on (blockchain, denom)
        ...
        coalesce(coingecko_symbol_total_volumes_in_usd, osmosis_symbol_total_volumes_in_usd, 0) as token_day_trading_volume
    FROM
...
SELECT
    ...
    coalesce(tp.coingecko_symbol_total_volumes_in_usd, tp.osmosis_symbol_total_volumes_in_usd, 0) as volume_in_usd
FROM
    flat.tokens as t
