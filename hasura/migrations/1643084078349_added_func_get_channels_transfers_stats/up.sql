DROP FUNCTION IF EXISTS public.get_channels_transfers_stats(integer, integer, boolean);

DROP TYPE IF EXISTS public.temp_t_get_channels_transfers_stats;

CREATE TYPE public.temp_t_get_channels_transfers_stats as (
    ...
    tx_in integer,
    tx_in_diff integer,
    tx_in_pending integer,
    tx_out integer,
    tx_out_diff integer,
    tx_out_pending integer,
    failed_tx integer,
    failed_tx_diff integer,
    chart jsonb
);
...
, transfer_chart as (
    select
        ...
        is_channel_calculable,
        is_counterparty_channel_calculable,
        json_agg(ibc_chart.txs) as chart
    from (
            select
                zone,
                counterparty_zone,
            ...
            from (
                ...
                FROM (
                    select
                        ...
                        series.hour,
                        coalesce(sum(ibc_data.success_txs), 0) AS txs
...
select
    ...
    coalesce(tx_in_pending.pending, 0)::integer as tx_in_pending,
    coalesce(counterparty_tx_in.success_txs, tx_out.success_txs, 0)::integer as tx_out,
    (coalesce(counterparty_tx_in.success_txs, tx_out.success_txs, 0) - coalesce(previous_counterparty_tx_in.success_txs, previous_tx_out.success_txs, 0))::integer as tx_out_diff,
    coalesce(counterparty_tx_in_pending.pending, 0)::integer as tx_out_pending,
...
left join current_ibc_pending_stats as tx_in_pending
    on channels.zone = tx_in_pending.zone and channels.channel = tx_in_pending.channel
left join current_ibc_pending_stats as counterparty_tx_in_pending
