DROP FUNCTION IF EXISTS public.get_channels_cashflow_stats(integer, integer, boolean);

DROP TYPE IF EXISTS public.temp_t_get_channels_cashflow_stats;

CREATE TYPE public.temp_t_get_channels_cashflow_stats as (
    ...
    cashflow_in_pending bigint,
    cashflow_out bigint,
    cashflow_out_diff bigint,
    cashflow_out_pending bigint,
    chart jsonb
);
...
with channel_info as (
    ...
    select
        ...
        coalesce(sum((stats.amount / POWER(10, tokens.symbol_point_exponent))::bigint * prices.coingecko_symbol_price_in_usd), 0)::bigint as usd_cashflow
    from
...
, current_cashflow_pending_stats as (
    select
        txout.zone as zone,
...
, cashflow_chart as (
    select
        ...
        is_channel_calculable,
        is_counterparty_channel_calculable,
        json_agg(ibc_chart.cashflow) as chart
    from (
            select
                zone,
                counterparty_zone,
            ...
            from (
                SELECT distinct
                    ...
                    hour,
                    cashflow,
                    row_number() OVER (PARTITION BY zone, channel order by hour) AS n
                FROM (
                    select
                        series.zone,
                        ...
                        series.hour,
                        coalesce(sum(ibc_data.usd_cashflow), 0) AS cashflow
                    from get_channels_series(period_in_hours) as series
                    left join current_hourly_cashflow_stats_in as ibc_data
                        on date_trunc('hour', ibc_data.datetime) = series.hour
                            and series.zone = ibc_data.zone
                            and series.channel = ibc_data.channel
                    group by
                        series.zone,
                        series.counterparty_zone,
                        series.channel,
...
select
    ...
    coalesce(tx_in_pending.pending, 0)::bigint as cashflow_in_pending,
    coalesce(counterparty_tx_in.usd_cashflow, tx_out.usd_cashflow, 0)::bigint as cashflow_out,
    (coalesce(counterparty_tx_in.usd_cashflow, tx_out.usd_cashflow, 0) - coalesce(previous_counterparty_tx_in.usd_cashflow, previous_tx_out.usd_cashflow, 0))::bigint as cashflow_out_diff,
    coalesce(counterparty_tx_in_pending.pending, 0)::bigint as cashflow_out_pending,
...
left join current_cashflow_pending_stats as tx_in_pending
    on channels.zone = tx_in_pending.zone and channels.channel = tx_in_pending.channel
left join current_cashflow_pending_stats as counterparty_tx_in_pending
    on channels.counterparty_zone = counterparty_tx_in_pending.zone and channels.counterparty_channel = counterparty_tx_in_pending.channel
...
left join previous_cashflow_stats_out as previous_counterparty_tx_out
