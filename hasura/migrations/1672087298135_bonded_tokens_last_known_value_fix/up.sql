...
, last_known_bonded as (
    SELECT
        pure.zone,
        (pure.bonded_tokens / POW(10, pt.symbol_point_exponent))::numeric as bonded_tokens
    FROM
        last_known_bonded_pure as pure
    LEFT JOIN public.zones on zones.chain_id = pure.zone
    LEFT JOIN public.tokens as pt on pt.zone = zones.chain_id and pt.base_denom = zones.base_token_denom
)
