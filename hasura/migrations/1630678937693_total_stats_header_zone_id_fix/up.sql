DROP FUNCTION IF EXISTS public.get_total_stats(integer, integer);
...
select
    (select count(chain_id)::int as zones_cnt_all from zones) as zones_cnt_all,
    (select count(*)::int from ibc_channels as all_channels) as channels_cnt_all,
