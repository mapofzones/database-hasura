...
, cashflow_mainnet_rating_weight as (
    select
        zone,
        -- rating mainnet
        row_number() OVER (Order By cashflow desc, zone asc)::int AS ibc_cashflow_mainnet_rating,
        row_number() OVER (Order By cashflow_in desc, zone asc)::int AS ibc_cashflow_in_mainnet_rating,
        row_number() OVER (Order By cashflow_out desc, zone asc)::int AS ibc_cashflow_out_mainnet_rating,
        -- rating mainnet diff
        (row_number() OVER (Order By cashflow - cashflow_diff desc, zone asc) - row_number() OVER (Order By cashflow desc, zone asc))::int as ibc_cashflow_mainnet_rating_diff,
        (row_number() OVER (Order By cashflow_in - cashflow_in_diff desc, zone asc) - row_number() OVER (Order By cashflow_in desc, zone asc))::int as ibc_cashflow_in_mainnet_rating_diff,
        (row_number() OVER (Order By cashflow_out - cashflow_out_diff desc, zone asc) - row_number() OVER (Order By cashflow_out desc, zone asc))::int as ibc_cashflow_out_mainnet_rating_diff,
        --weight mainnet
    ...
    from
        cashflow_additional_all_zones_data
    where
        is_mainnet = true
)
, cashflow_testnet_rating_weight as (
    select
        zone,
        -- rating testnet
        row_number() OVER (Order By cashflow desc, zone asc)::int AS ibc_cashflow_rating,
        row_number() OVER (Order By cashflow_in desc, zone asc)::int AS ibc_cashflow_in_rating,
        row_number() OVER (Order By cashflow_out desc, zone asc)::int AS ibc_cashflow_out_rating,
        -- rating testnet diff
        (row_number() OVER (Order By cashflow - cashflow_diff desc, zone asc) - row_number() OVER (Order By cashflow desc, zone asc))::int as ibc_cashflow_rating_diff,
        (row_number() OVER (Order By cashflow_in - cashflow_in_diff desc, zone asc) - row_number() OVER (Order By cashflow_in desc, zone asc))::int as ibc_cashflow_in_rating_diff,
        (row_number() OVER (Order By cashflow_out - cashflow_out_diff desc, zone asc) - row_number() OVER (Order By cashflow_out desc, zone asc))::int as ibc_cashflow_out_rating_diff,
        --weight testnet
...
, transfers_mainnet_rating_weight as (
    select
        zone,
        -- rating mainnet
        row_number() OVER (Order By tx desc, zone asc)::int AS ibc_tx_mainnet_rating,
        -- rating mainnet diff
        (row_number() OVER (Order By tx - tx_diff desc, zone asc) - row_number() OVER (Order By tx desc, zone asc))::int as ibc_tx_mainnet_rating_diff,
        --weight mainnet
...
, transfers_testnet_rating_weight as (
    select
        zone,
        -- rating testnet
        row_number() OVER (Order By tx desc, zone asc)::int AS ibc_tx_rating,
        -- rating testnet diff
        (row_number() OVER (Order By tx - tx_diff desc, zone asc) - row_number() OVER (Order By tx desc, zone asc))::int as ibc_tx_rating_diff,
        --weight testnet
