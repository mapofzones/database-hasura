...
CREATE TYPE public.temp_t_full_stats_for_each as (
    ...
    website varchar,
    ibc_active_addresses_mainnet integer,
    ibc_active_addresses_mainnet_diff integer,
    success_rate_mainnet numeric,
    ibc_cashflow_mainnet bigint,
    ibc_cashflow_mainnet_diff bigint,
    ibc_cashflow_in_mainnet bigint,
    ibc_cashflow_in_percent_mainnet numeric,
    ibc_cashflow_out_mainnet bigint,
    ibc_cashflow_out_percent_mainnet numeric,
    ibc_transfers_mainnet integer,
    ibc_transfers_mainnet_diff integer,
    ibc_transfers_pending_mainnet integer,
    ibc_cashflow_in_pending_mainnet bigint,
    ibc_cashflow_out_pending_mainnet bigint,
    ibc_cashflow_pending_mainnet bigint,
    ibc_cashflow_in_mainnet_diff bigint,
    ibc_cashflow_out_mainnet_diff bigint
);
...
-- mainnet transfers recalc start
, mainnet_transfers_zones_simple_data as ( -- only watcher-enabled mainnet zones
    select
        zone,
        sum (tx_in) as tx_in,
        sum (tx_in_diff) as tx_in_diff,
    ...
    from
        channels_transfers_tracked_stats as stats
    left join zones as zone_current on stats.zone = zone_current.chain_id
    left join zones as zone_counterparty on stats.counterparty_zone = zone_counterparty.chain_id
    where
        zone_current.is_mainnet = true
        and zone_counterparty.is_mainnet = true
    group by
        zone
)
, mainnet_transfers_additional_data as (
    select
        zone,
        tx_in + tx_out as tx,
        tx_in_diff + tx_out_diff as tx_diff,
    ...
    from
        mainnet_transfers_zones_simple_data
)
, mainnet_transfers_additional_all_zones_data as (
    select
        zones.chain_id as zone,
        is_mainnet,
        coalesce(tx, 0) as tx,
    ...
    from
        zones
    left join mainnet_transfers_additional_data as data
        on zones.chain_id = data.zone
)
-- mainnet transfers recalc end



-- mainnet cashflow recalc start
, mainnet_cashflow_zones_simple_data as (-- only watcher-enabled mainnet zones
    select
        zone,
        sum(cashflow_in) as cashflow_in,
        sum(cashflow_in_diff) as cashflow_in_diff,
...
, mainnet_cashflow_additional_data as (
    select
        zone,
        cashflow_in,
        cashflow_in_diff,
        ...
        END AS cashflow_out_percent
    from
        mainnet_cashflow_zones_simple_data
)
, mainnet_cashflow_additional_all_zones_data as (
    select
        zones.chain_id as zone,
        is_mainnet,
        coalesce(cashflow_in, 0) as cashflow_in,
    ...
    from
        zones
    left join mainnet_cashflow_additional_data as data
        on zones.chain_id = data.zone
)
-- mainnet cashflow recalc end

-- mainnet rating & weight recalc start
, transfers_mainnet_rating_weight as (
    select
        ...
        --weight mainnet
        tx::numeric / (select case when sum(cf.tx) = 0 then 1 else sum(cf.tx) end from mainnet_transfers_additional_all_zones_data as cf where cf.is_mainnet = true limit 1)::numeric as ibc_tx_mainnet_weight
    from
        mainnet_transfers_additional_all_zones_data
    where
        is_mainnet = true
)
, cashflow_mainnet_rating_weight as (
    select
        ...
        --weight mainnet
        cashflow::numeric / (select case when sum(cf.cashflow) = 0 then 1 else sum(cf.cashflow) end from mainnet_cashflow_additional_all_zones_data as cf where cf.is_mainnet = true limit 1)::numeric as ibc_cashflow_mainnet_weight,
        cashflow_in::numeric / (select case when sum(cf.cashflow_in) = 0 then 1 else sum(cf.cashflow_in) end from mainnet_cashflow_additional_all_zones_data as cf where cf.is_mainnet = true limit 1)::numeric as ibc_cashflow_in_mainnet_weight,
        cashflow_out::numeric / (select case when sum(cf.cashflow_out) = 0 then 1 else sum(cf.cashflow_out) end from mainnet_cashflow_additional_all_zones_data as cf where cf.is_mainnet = true limit 1)::numeric as ibc_cashflow_out_mainnet_weight
    from
        mainnet_cashflow_additional_all_zones_data as stats
    left join zones_statuses as statuses on statuses.zone = stats.zone
    where
        is_mainnet = true
)
-- mainnet rating & weight recalc end
...
select
    ...
    case when zones.is_active_addresses_hidden = true then NULL else calc.total_active_ibc_addresses::integer end as ibc_active_addresses,
    case when zones.is_active_addresses_hidden = true then NULL else calc.total_active_ibc_addresses_diff::integer end as ibc_active_addresses_diff,
    ...
    zones.website::varchar as website,
    case when zones.is_active_addresses_hidden = true then NULL else calc.total_active_ibc_addresses::integer end as ibc_active_addresses_mainnet,
    case when zones.is_active_addresses_hidden = true then NULL else calc.total_active_ibc_addresses_diff::integer end as ibc_active_addresses_mainnet_diff,
    mainnet_transfers.ibc_tx_success_rate::numeric as success_rate_mainnet,

    mainnet_cashflow.cashflow::bigint as ibc_cashflow_mainnet,
    mainnet_cashflow.cashflow_diff::bigint as ibc_cashflow_mainnet_diff,
    mainnet_cashflow.cashflow_in::bigint as ibc_cashflow_in_mainnet,
    mainnet_cashflow.cashflow_in_percent::numeric as ibc_cashflow_in_percent_mainnet,
    mainnet_cashflow.cashflow_out::bigint as ibc_cashflow_out_mainnet,
    mainnet_cashflow.cashflow_out_percent::numeric as ibc_cashflow_out_percent_mainnet,

    mainnet_transfers.tx::integer as ibc_transfers_mainnet,
    mainnet_transfers.tx_diff::integer as ibc_transfers_mainnet_diff,
    mainnet_transfers.tx_pending::integer as ibc_transfers_pending_mainnet,

    mainnet_cashflow.cashflow_in_pending::bigint as ibc_cashflow_in_pending_mainnet,
    mainnet_cashflow.cashflow_out_pending::bigint as ibc_cashflow_out_pending_mainnet,
    mainnet_cashflow.cashflow_pending::bigint as ibc_cashflow_pending_mainnet,
    mainnet_cashflow.cashflow_in_diff::bigint as ibc_cashflow_in_mainnet_diff,
    mainnet_cashflow.cashflow_out_diff::bigint as ibc_cashflow_out_mainnet_diff
from
    zones
...
left join mainnet_transfers_additional_all_zones_data as mainnet_transfers on zones.chain_id = mainnet_transfers.zone
left join mainnet_cashflow_additional_all_zones_data as mainnet_cashflow on zones.chain_id = mainnet_cashflow.zone;
